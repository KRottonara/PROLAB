...
class ExtendedKalmanFilter : public FilterNode
{
public:
    ExtendedKalmanFilter(ros::NodeHandle &nh);
    ~ExtendedKalmanFilter() override;
...
private:
    ...
    void reconfigCallback(example_package::ExtendedKalmanFilterConfig &config, uint32_t level);
    Eigen::Vector3d g(Eigen::Vector2d u_t1, Eigen::Vector3d mu_t0);


    // Extended Kalman filter-specific members
    Eigen::Matrix3d _G = Eigen::Matrix3d::Identity(3, 3);
    Eigen::Matrix3d _H = Eigen::Matrix3d::Identity(3, 3);
    ...
    Eigen::Vector2d _u_t1 = Eigen::Vector2d::Zero();

    dynamic_reconfigure::Server<example_package::ExtendedKalmanFilterConfig> dr_srv_;
    dynamic_reconfigure::Server<example_package::ExtendedKalmanFilterConfig>::CallbackType dr_cb_;
};